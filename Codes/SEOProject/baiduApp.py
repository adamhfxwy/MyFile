...
def search_keyword(driver, keyword, target_url):
    ...
    driver.get(base_url)
    search_box = driver.find_element("id", "kw")  # 百度搜索框的ID
    ...
    for page in range(1, 6):  # 只搜索前5页
        time.sleep(2)  # 等待结果加载
        results = driver.find_elements("css selector", 'div.result')  # 百度搜索结果的CSS选择器
        ...
        # 查找下一页按钮
        try:
            next_button = driver.find_element("css selector", "a.n")  # 百度下一页按钮的CSS选择器
            next_button.click()
        except:
            break
