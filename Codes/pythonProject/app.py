...
def setup_driver():
    ...
    driver = webdriver.Chrome(chrome_options=options)  # 使用 chrome_options 替换 options
    return driver
...
def search_keyword(driver, keyword, target_url):
    ...
    for page in range(1, 6):  # 只搜索前5页
        time.sleep(2)  # 等待结果加载
        results = driver.find_elements_by_css_selector('div.g')  # 使用 find_elements_by_css_selector

        for index, result in enumerate(results):
            try:
                link = result.find_element_by_tag_name('a').get_attribute('href')  # 使用 find_element_by_tag_name
        ...
        for keyword in keywords:
            if keyword in results:
                ...
                if page is not None and rank is not None:
                    file.write("{},@{}.{}\n".format(keyword, page, rank))  # 使用 format
                else:
...
def run_seo_script():
    ...
    for filename in os.listdir(input_folder):
        if filename.endswith(".txt"):  # 只处理txt文件
            ...
            output_file = os.path.join(output_folder, "results_{}".format(filename))  # 使用 format
