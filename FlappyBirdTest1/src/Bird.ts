 class Bird {
    public speedY: number = 0;      // 物体的垂直速度
    public gravity: number = 0.18;  // 重力加速度
    public groundY: number = 739;   // 地面的 y 坐标
    public isOnGround: boolean = false; // 判断物体是否在地面上
    private birdState:Laya.Image;
    private isMouseDown:boolean;
    private ceilingHeight: number = 0; // 天花板的高度
    private isFirst:boolean = true;
    private gameStartCallBackHandler:Laya.Handler;
    private gameOverCallBackHandler:Laya.Handler;
    public isGameOver: boolean = false; // 是否已经触发了 gameOver
    private rotationSpeed: number = 5;   // 翻滚的速度
    private isPlay:boolean=false;
    constructor(birdState:Laya.Image,gameStartCallBackHandler:Laya.Handler,gameOverCallBackHandler:Laya.Handler) {
    ...
    private gameOver():void{
      ...
      // 检测碰撞，检测两个矩形是否相交
    public checkCollision(column: Laya.Sprite): boolean {
        const birdBounds: Laya.Rectangle = this.birdState.getBounds();
        const columnBounds: Laya.Rectangle = column.getBounds();
        //处理图片边角与柱体之间的差值
        columnBounds.x+=60;
        ...
        // 检测碰撞，检测bird是否顺利通过
    public checkPass(column: Laya.Sprite): boolean {
        const birdBounds: Laya.Rectangle = this.birdState.getBounds();
